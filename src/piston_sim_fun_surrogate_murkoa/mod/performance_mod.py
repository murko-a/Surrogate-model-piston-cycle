...
def performance_fun(self, *models, perf_df, plot_perf):
    ...
    """
    self.mdls_pf = models
    performance_df, results_mae, results_mse, results_rmse, results_r2 = perf_calc(
        self)

    if perf_df:
        print(tabulate(performance_df, headers="keys", tablefmt="psql"))

    ordered_data_acc = performance_df.sort_values(
        by="Accuracy", ascending=True)
    ordered_data_mae = performance_df.sort_values(
        by="mean(MAE)", ascending=True)
    ordered_data_mse = performance_df.sort_values(
        by="mean(MSE)", ascending=True)
    ordered_data_rmse = performance_df.sort_values(
        by="mean(RMSE)", ascending=True)
    ...
    def plot_performance_lolipop(
        ...
        return plot_lolipop(
            self,
            od_acc=ordered_data_acc,
            od_mae=ordered_data_mae,
            od_mse=ordered_data_mse,
            od_rmse=ordered_data_rmse,
            od_r2=ordered_data_r2,
            od_time=ordered_data_time)
